...
EfErrCode ef_env_init(ef_env const *default_env, size_t default_env_size) {
	...
	printf("[KVDB]    KVDB start address is 0x%08X, size is %d bytes.\n", env_start_addr, KVDB_SIZE);
...
static char *find_env(const char *key) {
    ...
    if ((key == NULL) || *key == '\0') {
        printf("[KVDB]    Flash ENV name must be not empty!\n");
        return NULL;
    }
...
static EfErrCode create_env(const char *key, const char *value) {
    ...
    if (strchr(key, '=')) {
        printf("[KVDB]    Flash ENV name can't contain '='.\n");
        return EF_ENV_NAME_ERR;
    }

    /* find ENV */
    if (find_env(key)) {
        printf("[KVDB]    The name of \"%s\" is already exist.\n", key);
...
static EfErrCode del_env(const char *key) {
    ...
    if ((key == NULL) || *key == '\0') {
        printf("[KVDB]    Flash ENV name must be not NULL!\n");
        return EF_ENV_NAME_ERR;
    }

    if (strchr(key, '=')) {
        printf("[KVDB]    Flash ENV name or value can't contain '='.\n");
        return EF_ENV_NAME_ERR;
    }
...
EfErrCode ef_set_env(const char *key, const char *value) {
    ...
    if (!init_ok) {
        printf("[KVDB]    ENV isn't initialize OK.\n");
...
EfErrCode ef_load_env(void) {
    ...
    if ((env_end_addr == 0xFFFFFFFF) || (env_end_addr < env_start_addr)
        ...
        if (!env_crc_is_ok()) {
            printf("[KVDB]    Warning: ENV CRC check failed. Set it to default.\n");
            result = ef_env_set_default();
        }
    }
    return result;
...
EfErrCode ef_save_env(void) {
    ...
    switch (result) {
    case EF_NO_ERR: {
        printf("[KVDB]    Erased ENV OK.\n");
        break;
    }
    case EF_ERASE_ERR: {
        printf("[KVDB]    Error: Erased ENV fault! Start address is 0x%08X, size is %ld.\n", write_addr, write_size);
    ...
    switch (result) {
    case EF_NO_ERR: {
        printf("[KVDB]    Saved ENV OK.\n");
        break;
    }
    case EF_WRITE_ERR: {
        printf("[KVDB]    Error: Saved ENV fault! Start address is 0x%08X, size is %ld.\n", write_addr, write_size);
        break;
		default:
			break;
    }
...
static uint32_t calc_env_crc(void) {
    ...
    printf("[KVDB]    Calculate ENV CRC32 number is 0x%08X.\n", crc32);
...
static bool env_crc_is_ok(void) {
    if (calc_env_crc() == env_cache[ENV_PARAM_INDEX_DATA_CRC]) {
        printf("[KVDB]    Verify ENV CRC32 result is OK.\n");
        return true;
    } else {
        return false;
    }
