...
void lv_mbox_start_auto_close(lv_obj_t * mbox, uint16_t delay)
{
    ...
    if(ext->anim_time != 0) {
        ...
        lv_obj_animate(mbox, (lv_anim_builtin_t)LV_ANIM_GROW_H| LV_ANIM_OUT, ext->anim_time, delay, NULL);
        lv_obj_animate(mbox, (lv_anim_builtin_t)LV_ANIM_GROW_V| LV_ANIM_OUT, ext->anim_time, delay, (void (*)(lv_obj_t*))lv_obj_del);

        /*Disable fit to let shrinking work*/
        lv_cont_set_fit(mbox, false, false);
    } else {
        lv_obj_animate(mbox, (lv_anim_builtin_t)LV_ANIM_NONE, ext->anim_time, delay, (void (*)(lv_obj_t*))lv_obj_del);
    }
#else
    lv_obj_del(mbox);
#endif
