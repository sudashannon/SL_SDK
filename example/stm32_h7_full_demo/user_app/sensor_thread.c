...
{
    process_sema = osSemaphoreNew(1, 0, NULL);
    ...
    bool if_first_snap = true;
    for (;;) {
        int snap_result = sensor_snapshot(&sensor, &sensor_cap_image, 150);
        if (snap_result == 0) {
            if (if_first_snap == true)
                if_first_snap = false;
            else
                osSemaphoreAcquire(process_sema, osWaitForever);
