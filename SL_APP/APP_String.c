...
ustrncpy(char * restrict s1, const char * restrict s2, size_t n)
{
    ...
    // Check the arguments.
    //
    if(!s1)	APP_Assert((char *)__FILE__, __LINE__);
    if(!s2)	APP_Assert((char *)__FILE__, __LINE__);
           ...
           va_list arg)
{
    ...
    // Check the arguments.
    //
    if(!s) APP_Assert((char *)__FILE__, __LINE__);
    if(!n) APP_Assert((char *)__FILE__, __LINE__);
    if(!format) APP_Assert((char *)__FILE__, __LINE__);
...
ustrtoul(const char * restrict nptr, const char ** restrict endptr, int base)
{
    ...
    // Check the arguments.
    //
    if(!nptr) APP_Assert((char *)__FILE__, __LINE__);
    if(!(base == 0) || ((base > 1) && (base <= 16))) APP_Assert((char *)__FILE__, __LINE__);
...
int unumofstr(char *Mstr, char *substr)
{
	int number = 0;
	char *p;//×Ö·û´®¸¨ÖúÖ¸Õë
	char *q;//×Ö·û´®¸¨ÖúÖ¸Õë
...
char* uposofseg(char *seg,int segoffset,char *string)
{
	char *setpos;
	setpos = strstr(string,seg)+segoffset;
	return setpos;
}
/**
...
int32_t uparsenumber(char** str) {
	...
	while (STR_CHARISNUM(*p)) {                 /* Parse until character is valid number */
			val = val * 10 + STR_CHARTONUM(*p);
			p++;
	}
