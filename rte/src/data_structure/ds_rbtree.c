...
struct _rbt_s {
    rbt_node_t *root;
    rbt_cmp_callback_t cmp_keys_cb;
    rbt_free_value_callback_t free_value_cb;
};

rbt_t *
rbt_create(rbt_cmp_callback_t cmp_keys_cb, rbt_free_value_callback_t free_value_cb)
{
    rbt_t *rbt = rte_calloc(sizeof(rbt_t));
    if (!rbt)
        return NULL;
...
{
    int rc = 0;
    rbt_node_t *node = rte_calloc(sizeof(rbt_node_t));
    if (!node)
        return -1;

    node->key = rte_malloc(klen);
    if (!node->key) {
        rte_free(node);
        return -1;
    }
...
rbt_remove(rbt_t *rbt, void *k, size_t klen, void **v)
{
    ...
    if (node->left || node->right) {
        ...
        if (node->left && node->right) {
            ...
            void *new_key = rte_realloc(node->key, n->klen);
            if (!new_key)
                return -1;
            node->key = new_key;
