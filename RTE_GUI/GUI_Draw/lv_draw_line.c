        ...
        const lv_style_t * style, lv_opa_t opa_scale)
{
    ...
    if(MATH_ABS(point1->x - point2->x) > MATH_ABS(point1->y - point2->y)) {
    ...
    /*Arbitrary skew line*/
        ...
        if(main_line.hor) {
            if(main_line.p1.y < main_line.p2.y) {
                ...
                main_line.sy = MATH_ABS(main_line.sy);   /*The sign can change if the line becomes horizontal*/
            }
            ...
            if(main_line.p1.x < main_line.p2.x) {
                ...
                line_init(&main_line, &p1, &p_tmp);
                main_line.sx = MATH_ABS(main_line.sx);   /*The sign can change if the line becomes vertical*/
            }
            else if(main_line.p1.x > main_line.p2.x) {
                dir_ori = false;
                p_tmp.x = main_line.p2.x + 1;
...
static void line_init(line_draw_t * line, const lv_point_t * p1, const lv_point_t * p2)
{
    ...
    line->dx = MATH_ABS(line->p2.x - line->p1.x);
    line->sx = line->p1.x < line->p2.x ? 1 : -1;
    line->dy = MATH_ABS(line->p2.y - line->p1.y);
