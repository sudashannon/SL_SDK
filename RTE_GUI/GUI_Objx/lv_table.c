...
#include "GUI_Port.h"
#include "GUI_Draw/lv_draw_label.h"
...
static lv_coord_t get_row_height(lv_obj_t * table, uint16_t row_id)
{
    ...
    for(cell = row_start, col = 0; cell < row_start + ext->col_cnt; cell++, col ++) {
        if(ext->cell_data[cell] != NULL) {
            ...
            if(format.crop) {
                h_max = MATH_MAX(lv_font_get_height(cell_style->text.font) + 2 * cell_style->body.padding.ver, h_max);
            }
            /*Without text crop calculate the height of the text in the cell*/
                ...
                h_max = MATH_MAX(txt_size.y + 2 * cell_style->body.padding.ver, h_max);
                cell += col_merge;
                col += col_merge;
            }
        }
