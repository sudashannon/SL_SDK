...
static bool lv_lmeter_design(lv_obj_t * lmeter, const lv_area_t * mask, lv_design_mode_t mode)
{
    ...
    /*Draw the object*/
        ...
        for(i = 0; i < ext->line_cnt; i++) {
            ...
            lv_coord_t y_out = (int32_t)((int32_t)MATH_TrigoSin(angle) * r_out) >> LV_TRIGO_SHIFT;
            lv_coord_t x_out = (int32_t)((int32_t)MATH_TrigoSin(angle + 90) * r_out) >> LV_TRIGO_SHIFT;
            lv_coord_t y_in = (int32_t)((int32_t)MATH_TrigoSin(angle) * r_in) >> LV_TRIGO_SHIFT;
            lv_coord_t x_in = (int32_t)((int32_t)MATH_TrigoSin(angle + 90) * r_in) >> LV_TRIGO_SHIFT;
...
static lv_res_t lv_lmeter_signal(lv_obj_t * lmeter, lv_signal_t sign, void * param)
{
    ...
    if(sign == LV_SIGNAL_CLEANUP) {
        ...
        lmeter->ext_size = MATH_MAX(lmeter->ext_size, style->line.width);
