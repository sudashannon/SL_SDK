...
static bool lv_arc_design(lv_obj_t * arc, const lv_area_t * mask, lv_design_mode_t mode)
{
    ...
    /*Draw the object*/
        ...
        lv_coord_t r = (MATH_MIN(lv_obj_get_width(arc), lv_obj_get_height(arc))) / 2;
        ...
        if(style->line.rounded) {
            ...
            lv_coord_t cir_x = ((r - thick_half) * MATH_TrigoSin(ext->angle_start) >> LV_TRIGO_SHIFT);
            lv_coord_t cir_y = ((r - thick_half) * MATH_TrigoSin(ext->angle_start + 90) >> LV_TRIGO_SHIFT);
            ...
            cir_x = ((r - thick_half) * MATH_TrigoSin(ext->angle_end) >> LV_TRIGO_SHIFT);
            cir_y = ((r - thick_half) * MATH_TrigoSin(ext->angle_end + 90) >> LV_TRIGO_SHIFT);
