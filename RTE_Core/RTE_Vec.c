/**
  ...
  * @brief   动态数组实现。
  * @version V1.0 2018/9/30 第一版
	* @History V1.0 创建，复制自 https://github.com/rxi/vec
  ******************************************************************************
  */
#include "RTE_Vec.h"
...
int vec_expand_(char **data, int *length, int *capacity, int memsz)
{
  if (*length + 1 > *capacity)
    ...
    ptr = Memory_Realloc(BANK_RTE,*data, n * memsz);
...
int vec_reserve_(char **data, int *length, int *capacity, int memsz, int n)
{
  ...
  if (n > *capacity)
	{
    void *ptr = Memory_Realloc(BANK_RTE,*data, n * memsz);
    if (ptr == NULL) return -1;
    *data = ptr;
    *capacity = n;
  }
